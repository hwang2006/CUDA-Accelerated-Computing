...
void run_performance_analysis() {
    int step_size = 500;  // Define how much to increase problem size
    int max_size = 4000;  // Maximum size of the problem to test
    ...
    for (int size = step_size; size <= max_size; size += step_size) {
        ...
        printf("Running for H = W = %d...\n", size);
        printf("Running for N = %d, C = %d, K = %d, R = %d, S = %d...\n", N, C, K, R, S);
        float *I, *F, *O;
