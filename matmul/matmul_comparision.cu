...
__global__ void MatMul_xCol(float* matA, float* matB, float* matC, int m, int n, int k);
__global__ void MatMul_shared_xCol(float* matA, float* matB, float* matC, int m, int n, int k);
...
int main(int argc, char* argv[])
{
	...
	timer.setTimerName(1, "Kernel with row - basic");
	timer.setTimerName(3, "Kernel - shared memory");
	timer.setTimerName(5, "Kernel - no bank conflict");
	timer.setTimerName(6, "[Data transter] host->device");
	timer.setTimerName(7, "[Data transfer] device->host");

    timer.setTimerName(2, "Kernel with col - basic");
    timer.setTimerName(4, "Kernel - shared memory with col");
    ...
    int ROW_SIZE = ceil((float)m / BLOCK_SIZE);
    int COL_SIZE = ceil((float)n / BLOCK_SIZE);

	dim3 blockDim(BLOCK_SIZE, BLOCK_SIZE);
    dim3 gridDim(ROW_SIZE, COL_SIZE);
    ...
    gridDim.x = ROW_SIZE;
    gridDim.y = COL_SIZE;
    MatMul_xCol << <gridDim, blockDim >> > (dA, dB, dC, m, n, k);
    checkCudaErrors(cudaDeviceSynchronize());
    timer.offTimer(2);

    checkCudaErrors(cudaMemcpy(Cgpu, dC, sizeC * sizeof(float), cudaMemcpyDeviceToHost));
#ifdef DO_CPU
    printf("[Kernel basic with Col] ");
    compareMatrix(Ccpu, Cgpu, sizeC);
#endif
    ...
    gridDim.x = ROW_SIZE;
    gridDim.y = COL_SIZE;
    MatMul_shared_xCol << <gridDim, blockDim >> > (dA, dB, dC, m, n, k);
    checkCudaErrors(cudaDeviceSynchronize());
    timer.offTimer(4);

    checkCudaErrors(cudaMemcpy(Cgpu, dC, sizeC * sizeof(float), cudaMemcpyDeviceToHost));
    #ifdef DO_CPU
        printf("[Kernel shared memory with xCol] ");
        compareMatrix(Ccpu, Cgpu, sizeC);
    #endif
...
bool compareMatrix(float* _A, float* _B, int _size)
{
    ...
    //LOOP_I(10) printf("[%d]: (%f, %f)\n", i, _A[i], _B[i]);// print out the first 10s
    //[0]: (25873.453125, 25873.453125)
    //[1]: (26409.310547, 26409.310547)
    //[2]: (25610.015625, 25610.015625)
    //...
    //[9]: (26063.822266, 26063.822266)
